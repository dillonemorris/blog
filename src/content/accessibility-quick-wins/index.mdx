...
In Chrome for example, when you open the developer tools (Cmd + Option + i) you'll just want to make sure you have the element inspector selected:
...
The DOM is an API we can use as developers to manipulate the visual representation of those elements that the user interacts with. The word visual here is crucial because the DOM tree is really only useful to sighted users, at least directly. The browser takes this DOM tree and creates an accessibility tree based off of it. In turn, assistive technologies can use this accessibility tree to form a representation of the UI that visually impaired users can understand.
...
The web browser will first take this structure and create a DOM tree that looks like this:
...
Now, a lot of the information in a DOM tree is not useful to visually impaired users. So what the browser does from here is create an accessibility tree based off of this tree keeping only the parts that are useful for assistive technologies. That tree might look something like this:
...
We want to use HTML as it's intended. If we don't do so we lose out on the accessibility benefits associated with doing it the correct way.
...
The alt text is like a fallback that can be shown if the image isn't rendered properly or a screen reader is being used:
...
The best way is to use a screen reader to test your site and make sure you can navigate it. You should do this. Additionally, or to get a quick temperature
check on the state of accessibility on your site is to use a friendly tool that comes with Google Chrome called Lighthouse.
...
Not only that, you are presented with some opportunity areas for improvement. Below is the report for this blog as of the writing of this post:
...
It's really handy that google provides this, it's really a labor of love.
